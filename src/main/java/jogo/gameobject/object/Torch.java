...
public class Torch extends GameObject implements LightSource{
    ...
    public Spatial getSpatial(AssetManager assetManager){
        ...
        geometries[0].setLocalTranslation(0f, 0.4f, 0.5f);
        geometries[0].setLocalRotation(new Quaternion(0.25f,0f,0f,1f));

        geometries[1].setLocalTranslation(-0f, 0.4f, -0.5f);
        geometries[1].setLocalRotation(new Quaternion(-0.25f,0f,0f,1f));

        geometries[2].setLocalTranslation(-0.5f, 0.4f, 0f);
        geometries[2].setLocalRotation(new Quaternion(0f,0f,0.25f,1f));

        geometries[3].setLocalTranslation(0.5f, 0.4f, 0f);
    ...
    public PointLight[] getLight(){
        ...
        Vector3f[] lPositions = new Vector3f[]{new Vector3f(-0f, 0.6f, -0.8f), new Vector3f(-0f, 0.6f, 0.8f),new Vector3f(-0.8f, 0.6f, 0),new Vector3f(0.8f, 0.6f, 0)};
        for (int i = 0; i < lights.length; i++) {
            lights[i].setPosition(lPositions[i].add(getPosition().toVector3f()));
            lights[i].setColor(ColorRGBA.fromRGBA255(120, 90, 42, 255));
            lights[i].setRadius(2.5f);
        }
